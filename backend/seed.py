...
def seed():
    client = MongoClient(os.getenv('MONGO_URI', 'mongodb://localhost:27017/library-system'))
    db = client.get_default_database()

    db.users.delete_many({})
    db.collectionitems.delete_many({})
    db.usagerecords.delete_many({})

    print('Cleared existing data')

    # Create admin
    admin_pw = bcrypt.hashpw('admin123'.encode('utf-8'), bcrypt.gensalt(12))
    db.users.insert_one({
        'name': 'Admin User',
    ...
    # Create users
    user_names = [
        'Juan Dela Cruz', 'Maria Santos', 'Jose Rizal', 'Ana Gonzales',
        'Carlos Mendoza', 'Sofia Lopez', 'Miguel Tan', 'Angela Cruz',
        'Ricardo Bautista', 'Elena Garcia', 'Roberto Santos', 'Lisa Chen',
        'Paulo Reyes', 'Sarah Lim', 'Antonio Reyes', 'Maria Flores',
        'Dave Torres', 'Amelia Lopez', 'Luis Reyes', 'Carla Villanueva'
    ]
    user_ids = []
    for i, name in enumerate(user_names):
        pw = bcrypt.hashpw('password123'.encode('utf-8'), bcrypt.gensalt(12))
        result = db.users.insert_one({
            'name': name,
            'email': f'user{i + 1}@university.edu',
            'password': pw,
    ...
    for i in range(150):
        ...
        result = db.collectionitems.insert_one({
            ...
            'publisher': publisher,
            'location': f'Shelf {chr(65 + (i % 6))}-{random.randint(1, 20)}',
    ...
    print('Created collection items')

    # Create usage records
    for _ in range(500):
        item_id = random.choice(item_ids)
        user_id = random.choice(user_ids)
        borrow_date = random_date(datetime(2023, 1, 1), datetime.now())
        return_date = borrow_date + timedelta(days=random.randint(1, 30))
        renewal_count = random.randint(0, 3)
        dwell_time = (return_date - borrow_date).days

        user = db.users.find_one({'_id': user_id})

        db.usagerecords.insert_one({
            'collectionItem': item_id,
            'user': user_id,
            'borrowDate': borrow_date,
            'returnDate': return_date,
            'dueDate': borrow_date + timedelta(days=14),
